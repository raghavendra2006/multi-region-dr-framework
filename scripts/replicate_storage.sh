...
aws --endpoint-url=http://localhost:4566 s3 cp "s3://${PRIMARY_BUCKET_NAME}/${LATEST}" "$TEMP_FILE"

# Upload the backup to DR region bucket
echo "INFO: Uploading $LATEST to DR region bucket s3://${DR_BUCKET_NAME}/..."
aws --endpoint-url=http://localhost:4567 s3 cp "$TEMP_FILE" "s3://${DR_BUCKET_NAME}/${LATEST}"
